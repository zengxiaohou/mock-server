...
const { extractQueryParams } = require('./tools');
...
function getMockConfig(url, ctx) {
  ...
  const result = matchPathWithRegex(mock.data, id, ctx);
  for (let i = 0; i < result.length; i++) {
    const { apis, businessKey, mockKey } = result[i];
    const checked = mockChecked[businessKey][mockKey];
    ...
    // 条件匹配的mock数据选择，优先匹配string query，再匹配body
    if (api.list) {
      return (
        api.list.find(item => {
          const mockConditions = extractQueryParams(item.filename);
          let isMatch = true;
          Object.keys(mockConditions).forEach(key => {
            if (
              mockConditions[key] !== url.searchParams.get(key) &&
              mockConditions[key] !== ctx.request.body[key]
            ) {
              isMatch = false;
            }
          });
...
function matchPathWithRegex(mocks, id, ctx) {
  const result = [];
  Object.keys(mocks).forEach(businessKey => {
    let mockKey = '';
    const apis = _.find(mocks[businessKey], (m, key) => {
      const paramsKey = [];
    ...
    result.push({ apis, businessKey, mockKey });
  });
  return result;
}
