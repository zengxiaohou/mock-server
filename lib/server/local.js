 ...
 * @param {*} rules 场景规则集
 */
function createSet(name, rules) {
  const { dir } = getStatus();
  const content = {
    setName: name,
    rules
  };
  const setRootPath = path.resolve(dir, '_setV2');
  fs.writeFileSync(`${setRootPath}/${name}.json`, JSON.stringify(content));
}
